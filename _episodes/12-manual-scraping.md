...
Note that the profiles on the Jewish Studies facultly page are laid out differently from the East Asian Studies faculty page. On the Jewish Studies page the information is not displayed in well-defined rows. So, when we scrape the web page data, we should get one row per faculty with one string of data.
